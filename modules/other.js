...
function Rmanageequality() {
    ...
    if (!(game.global.challengeActive == "Exterminate" && getPageSetting('Rexterminateon') == true && getPageSetting('Rexterminateeq') == true && !game.global.mapsActive)) {
        if (fastimps.includes(getCurrentEnemy().name) || (game.global.mapsActive && getCurrentMapObject().location == "Void") || (game.global.gridArray[game.global.lastClearedCell+2].u2Mutation.length > 0) || (game.global.mapGridArray[game.global.lastClearedMapCell+2].u2Mutation.length > 0 )) {
